...
Citizen.CreateThread(function()
	while true do
		...
		if IsPlayerInPolmav() then
			...
			if IsControlJustPressed(0, toggle_display) and GetPedInVehicleSeat(heli, 2) == lPed then 
				ChangeDisplay()
			end
		...
		if helicam then
			...
			while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) and GetPedInVehicleSeat(heli, 1) == lPed do
				if IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
					PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
